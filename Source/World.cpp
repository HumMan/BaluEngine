...
bool TBaluWorld::ObjectNameExists(TWorldObjectType type, const char* name)
{
	...
	//TODO rename - после перехода на shared_ptr не нужно будет заботитьс€ о том что объект map был перемещен и нужно обновл€ть указатели в остальных местах (world_instance например)
}
...
TBaluMaterial* TBaluWorld::GetMaterial(const char* scene_name)
{
	...
	{
		throw std::invalid_argument("ћатериал с данным имененем отсутсвует");
	}
}
...
TBaluSprite* TBaluWorld::GetSprite(const char* scene_name)
{
	...
	{
		throw std::invalid_argument("—прайт с данным имененем отсутсвует");
	}
}
...
TProperty* PropertiesFactory::Create(const char* name)
{
	...
	throw std::invalid_argument("“ип не зарегистрирован");
}
