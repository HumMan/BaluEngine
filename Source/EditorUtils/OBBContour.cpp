...
EngineInterface::IBaluClass* TOBBContour::CreateClass(EngineInterface::IBaluWorld* world, EngineInterface::IBaluScene* scene, TOBBContourPrivate* data)
{
	...
	adornment_sprite->GetPolygone()->AddOnCustomDraw(CallbackWithData<TCustomDrawCallback>(OBBContourCustomDraw, &world->GetCallbacksActiveType(), data, TCallbacksActiveType::EDITOR));
