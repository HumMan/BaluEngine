		...
		Matrix Transform;
						...
						void SetHullTolerance(float value)
						{
							...
							else
							{
								...
								TextureConverter()
									{
										Initialize(nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
									}

									TextureConverter(char ? alphaTolerance, float ? hullTolerance,
										bool ? holeDetection, bool ? multipartDetection, bool ? pixelOffsetOptimization, Matrix ? transform)
									{
										Initialize(nullptr, nullptr, alphaTolerance, hullTolerance, holeDetection,
											multipartDetection, pixelOffsetOptimization, transform);
									}

									TextureConverter(unsigned int[] data, int width)
									{
										Initialize(data, width, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
									}

									TextureConverter(unsigned int* data, int width, char ? alphaTolerance,
									...
									private:
										void Initialize(unsigned int[] data, int ? width, char ? alphaTolerance,
										float ? hullTolerance, bool ? holeDetection, bool ? multipartDetection,
										bool ? pixelOffsetOptimization, Matrix ? transform)
									{
										if (data != nullptr && !width.HasValue)
											throw new ArgumentNullException("width", "'width' can't be nullptr if 'data' is set.");

										if (data == nullptr && width.HasValue)
											throw new ArgumentNullException("data", "'data' can't be nullptr if 'width' is set.");
									...
									private void SetTextureData(unsigned int[] data, int width)
									{
										if (data == nullptr)
											throw new ArgumentNullException("data", "'data' can't be nullptr.");
									...
									/// <returns></returns>
									public static Vertices DetectVertices(unsigned int[] data, int width)
									{
									...
									/// <returns></returns>
									public static Vertices DetectVertices(unsigned int[] data, int width, bool holeDetection)
									{
										TextureConverter tc =
									...
									/// <returns></returns>
									public static List<Vertices> DetectVertices(unsigned int[] data, int width, float hullTolerance, char alphaTolerance, bool multiPartDetection, bool holeDetection)
									{
										TextureConverter tc =
											new TextureConverter(data, width)
										{
									...
									public List<Vertices> DetectVertices()
									{
										...
										if (_data == nullptr)
											throw new Exception("'_data' can't be nullptr. You have to use SetTextureData(unsigned int[] data, int width) before calling this method.");
										...
										if (_data.Length % _width != 0)
											throw new Exception("'_width' has an invalid value. You have to use SetTextureData(unsigned int[] data, int width) before calling this method.");

										#endregion

										List<Vertices> detectedPolygons = new List<Vertices>();

										Vector2 ? holeEntrance = nullptr;
										Vector2 ? polygonEntrance = nullptr;
													...
													do
													{
														...
														if (holeEntrance.HasValue)
														{
															if (!blackList.Contains(holeEntrance.Value))
															{
																...
																if (holePolygon != nullptr && holePolygon.Count > 2)
																{
										...
										if (detectedPolygons == nullptr || (detectedPolygons != nullptr && detectedPolygons.Count == 0))
									...
									private void ApplyTriangulationCompatibleWinding(ref List<Vertices> detectedPolygons)
									{
										for (int i = 0; i < detectedPolygons.Count; i++)
										{
											...
											if (detectedPolygons[i].Holes != nullptr && detectedPolygons[i].Holes.Count > 0)
											{
									...
									{
										if (polygon == nullptr)
											throw new ArgumentNullException("'polygon' can't be nullptr.");
									...
									{
										if (polygon == nullptr)
											throw new ArgumentNullException("polygon", "'polygon' can't be nullptr.");
									...
									private bool SplitPolygonEdge(Vertices polygon, Vector2 coordInsideThePolygon, out int vertex1Index, out int vertex2Index)
									{
										...
										if (xCoords != nullptr && xCoords.Count > 1 && xCoords.Count % 2 == 0)
										{
											float distance;
