...
bool TEventsEditorInstance::CompileScripts(std::string& error_message)
{
	try
	{
		for (auto& v : source->GetOnWorldStart())
		{
		...
		for (auto& v : source->GetOnViewportResize())
		{
			auto method_body = v.GetScriptSource();
...
TEventsEditorInstance::TEventsEditorInstance(IEventsEditor* source, std::string assets_dir)
{
	this->source = source;
